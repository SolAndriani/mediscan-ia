...
export default function Dashboard() {
  ...
  const simulateUpload = async (filename: string, imageBase64?: string) => {
    ...
    // Si hay imagen real, llamamos a la API
    if (imageBase64) {
      setAnalyzing(true)
      try {
        ...
        setAiResult(['No se pudo conectar con la IA. Usando modo demo.'])
      }
      setAnalyzing(false)
    }
              ...
              {!uploading ? (
                <div style={{ padding:'1rem 1.25rem' }}>
                  <div onDragOver={e=>e.preventDefault()} onDrop={handleDrop} onClick={() => fileRef.current?.click()}
                    style={{ border:'1.5px dashed var(--border2)', borderRadius:10, padding:'2rem', textAlign:'center', cursor:'pointer', background:'var(--teal-glow, rgba(29,158,117,.05))', transition:'all .2s' }}>
                  ...
                  {uploadSteps.map((step, i) => {
                    ...
                    return (
                      <div key={i} style={{ display:'flex', alignItems:'center', gap:'.75rem', fontSize:'.82rem' }}>
                        ...
                        <div style={{ color: active?'var(--text)': done?'var(--text3)':'var(--text3)', textDecoration: done?'line-through':'none' }}>{step}</div>
                      </div>
                    )
                  })}
                </div>
          ...
          <div style={{ width:320, flexShrink:0, display:'flex', flexDirection:'column', gap:'1.25rem' }}>
            ...
            <div style={S.card}>
              ...
              <div style={{ padding:'0 1.25rem 1rem', display:'flex', flexDirection:'column', gap:'.5rem' }}>
                {analyzing && <div style={{ fontSize:'.8rem', color:'var(--teal-light)', padding:'.5rem' }}>🔍 Analizando imagen con IA...</div>}
                ...
                {(!analyzing && (aiResult.length===0) && (current?.findings||[]).length===0) && (
                  <div style={{ fontSize:'.8rem', color:'var(--text3)', padding:'.5rem' }}>Sin hallazgos disponibles</div>
                )}
              </div>
            </div>
            ...
            <div style={S.card}>
              ...
              <div style={{ padding:'0 1.25rem 1rem', fontSize:'.8rem', color:'var(--text2)', lineHeight:1.8 }}>
                ...
                <p>Radiografía de tórax PA, bipedestación, inspiración. Buena técnica radiológica.</p>
                <div style={{ fontSize:'.68rem', textTransform:'uppercase', letterSpacing:'.08em', color:'var(--text3)', margin:'.8rem 0 .4rem' }}>Hallazgos</div>
                <p>Imagen de densidad aumentada en lóbulo superior derecho de ~2.3 cm. Silueta cardíaca normal. Senos costofrénicos libres.</p>
                <div style={{ fontSize:'.68rem', textTransform:'uppercase', letterSpacing:'.08em', color:'var(--text3)', margin:'.8rem 0 .4rem' }}>Conclusión IA</div>
                <p>Requiere correlación clínica. Se recomienda TAC con contraste.</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div style={{ background:'var(--bg2)', border:'1px solid var(--border2)', borderRadius:16, width:480, maxWidth:'95vw', padding:'2rem' }}>
            ...
            <div style={{ marginBottom:'1rem' }}>
              ...
              <select value={form.type} onChange={e => setForm(f=>({...f,type:e.target.value}))}
                style={{ width:'100%', background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:7, padding:'.6rem .85rem', fontSize:'.85rem', fontFamily:'inherit', color:'var(--text)', outline:'none' }}>
                {['RX Tórax','TAC Abdomen','Ecografía Abdominal','Mamografía','RMN Columna'].map(t=><option key={t}>{t}</option>)}
              </select>
            </div>
